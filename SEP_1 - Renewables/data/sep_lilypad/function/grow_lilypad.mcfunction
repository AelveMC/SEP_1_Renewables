...
execute at @e[type=minecraft:armor_stand,tag=sep_temp_lily] run execute if items entity @n[type=item,distance=..0] container.0 minecraft:dirt[minecraft:custom_data={sep_lily:true}] if block ~ ~-1 ~ water if block ~ ~ ~ air run setblock ~ ~ ~ minecraft:lily_pad
