...
#	if it has burnt in sunlight for at least one second, AND
#	if it has sky access at that one second, AND
#	if it has nothing on its head, 
# then it will be tagged.
execute unless score $undead_burn_dummy sep_undead_burn_time matches 12541..23460 if score @s sep_zombie_burn_timer matches 20 if predicate sep_sand:check_sky unless entity @s[nbt={ArmorItems: [{}, {}, {}, {count: 1}]}] unless predicate sep_sand:check_raining run tag @s add sep_sunburnt
